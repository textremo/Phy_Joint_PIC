...
masks = zeros(trials, nt, 1);
mask_rows = zeros(trials, 1, nt);
xs = zeros(trials, nt, 1);
Hs = zeros(trials, nr, nt);
ys = zeros(trials, nr, 1);

x_hat1s = zeros(trials, nt, 1);
x_hat1_ms = zeros(trials, nt, 1);
x_hat2s = zeros(trials, nt, 1);
x_hat2_ms = zeros(trials, nt, 1);
...
for i = 1:trials
    ...
    ys(i, :, :) = y;
    x_hat1s(i, :, :) = x_hat1;
    x_hat1_ms(i, :, :) = x_hat1_m;
    x_hat2s(i, :, :) = x_hat2;
    x_hat2_ms(i, :, :) = x_hat2_m;
...
save(path + "lmmse_mask_iter_n.mat");
fprintf("\nLMMSE (mask) iter n: data is generated.\n");
